...
SELECT orders.id, orders.status, orders.order_total FROM orders INNER JOIN customers ON orders.customer_id=customers.id AND customers.email='pclark74@gmail.com';
...
SELECT common_name, melon_type, quantity, unit_price, total_price FROM melons INNER JOIN order_items ON order_items.melon_id=melons.id WHERE order_items.order_id=2725;
